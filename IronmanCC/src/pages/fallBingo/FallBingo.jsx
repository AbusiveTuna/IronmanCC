...
const FallBingo = ({
    ...
    const rowCompletedA = useMemo(
        () =>
            rows.map((row) =>
                row.filter((t) => t && t.Type !== "passive").every((t) => isComplete(mapA[t.Id], t.Goal))
            ),
        [rows, mapA]
    );
    const rowCompletedB = useMemo(
        () =>
            rows.map((row) =>
                row.filter((t) => t && t.Type !== "passive").every((t) => isComplete(mapB[t.Id], t.Goal))
            ),
        [rows, mapB]
    );

    const visibleRows = useMemo(() => {
        const visA = 1 + rowCompletedA.findIndex((v) => !v);
        const visB = 1 + rowCompletedB.findIndex((v) => !v);
        const a = visA === 0 ? ROWS : Math.min(visA, ROWS);
        const b = visB === 0 ? ROWS : Math.min(visB, ROWS);
        return Math.max(a, b);
    }, [rowCompletedA, rowCompletedB]);
            ...
            {!!petTiles.length && (
                <div className="FallBingo-pets">
                    {petTiles.map((t) => {
                        ...
                        const current = claimedBy === "A" ? currentOf(a) : claimedBy === "B" ? currentOf(b) : Math.max(currentOf(a), currentOf(b));
